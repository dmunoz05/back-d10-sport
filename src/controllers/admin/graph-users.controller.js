...
export const getAllUsers = async (req, res) => {
    const conn = await getConnection();
    const db = variablesDB.academy;
    const select = await conn.query(`SELECT id_user, first_names, last_names, gender, city, contact, mail, social_networks FROM ${db}.athlete
UNION
SELECT id_user, first_names, last_names, gender, city, contact, mail, social_networks FROM ${db}.coach`, []);
    if (!select) return res.json(responseQueries.error({ message: "Error obteniendo las clases" }));
    return res.json(responseQueries.success({ data: select[0] }));
}